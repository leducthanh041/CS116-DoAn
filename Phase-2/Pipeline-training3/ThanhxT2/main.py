...
import json # [FIX] Đã thêm thư viện json
...
# ====================================================
# [AUTO] LOAD FEATURE CONFIG TỪ STAGE 1
# ====================================================
def get_stage2_features():
    """Load danh sách feature tự động từ file config do Stage 1 tạo ra"""
    config_path = get_path("artifacts/feature_config.json")
    
    # [DEFAULT] Danh sách feature chuẩn (Backup nếu chưa có file json)
    # Khớp với phiên bản Stage 1 "Clean"
    default_features = [
    ...
    if Path(config_path).exists():
        try:
            with open(config_path, "r") as f:
                feats = json.load(f)
            print(f">>> [Auto-Config] Loaded {len(feats)} features from JSON.")
            return feats
        except Exception as e:
            print(f">>> [Warning] Error loading feature json: {e}. Using default.")
            return default_features
    else:
        print(">>> [Info] feature_config.json not found (Stage 1 not run yet?). Using default.")
        return default_features

# Load danh sách feature ngay khi chạy script
STAGE2_FEATURE_COLS = get_stage2_features()
...
def run_pipeline_flow(
    ...
    # Kiểm tra cache model và feature list
    if Path(model_path).exists() and Path(feat_path).exists():
        ...
        # [FIX] Truyền danh sách feature_cols cụ thể vào đây
        # Để đảm bảo model chỉ học trên các cột clean chúng ta muốn
...
def main():
    ...
    if Path(stage1_val_path).exists():
        ...
        stage1_val.fit(df_train) # Khi fit xong sẽ tự save feature_config.json
