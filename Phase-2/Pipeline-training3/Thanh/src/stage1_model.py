...
class ItemItemCFStage1(BaseEstimator):
    def __init__(self, config, df_item, df_user):
        ...
        # [REMOVED] self.user_item_last_date_ = {} 
        ...
        # [UPDATED] Feature Names List (Removed item_hist_cnt, feat_days_since_item)
        self.feature_names = [
            "stage1_score", "stage1_rank", "sim_max", "sim_avg", "support_cnt",
            "brand_match_cnt", "cat2_match_cnt",
            "feat_days_since_cat", 
    ...
    def _load_age_features(self):
        """Hàm load dữ liệu tuổi từ file Parquet"""
    ...
    def fit(self, df_train):
        print(f"[{self.__class__.__name__}] Starting fit process (Refactored)...")
    ...
    def _build_advanced_features_data(self):
        print("   -> Building metadata & user profiles (Warm Features)...")
        cols = ["item_id"]
        ...
        # 1. Category Last Date
        print("      Calculating Category Last Dates...")
        cat_last_date_series = df_valid.groupby(["customer_id", "category_l2"])["created_datetime"].max()
        self.user_cat_last_date_ = cat_last_date_series.to_dict() # Key: (uid, cat2)
        
        # [REMOVED] Item Last Date calculation (removed user_item_last_date_)
        ...
        for uid, group in tqdm(df_valid.groupby("customer_id"), desc="Building Profiles"):
            ...
            # Tính mức chi tiêu trung bình cho từng category
    ...
    def _build_essential_cold_start_list(self):
        ...
        essential_keywords = ["SNACK", "CHÁO", "SỮA", "TÃ", "BỈM", "KHĂN ƯỚT", "VỆ SINH", "THỰC PHẨM", "BÁNH GẠO", "NUTIFOOD", "BÌNH SỮA", "TẮM GỘI", "DẦU ĂN", "PHÔ MAI", "NÚM TY"]
        top_brands = ["ANIMO", "HOFF", "SÀI GÒN FOOD", "SG FOOD", "IVENET", "CÂY THỊ", "TAKATO", "BOBBY", "PIGEON", "VINAMILK", "GERBER", "GRINNY", "BEBEDANG"]
        
        candidate_items = []
        for idx, meta in self.item_metadata_.items():
            score = 0
            cat_str = str(meta.get("cat", "")).upper() + " " + str(meta.get("cat2", "")).upper()
            if any(k in cat_str for k in essential_keywords): score += 1.5
            brand_str = str(meta.get("brand", "")).upper()
            if any(b in brand_str for b in top_brands): score += 2.0
        ...
        self.essential_cold_indices_ = [x[0] for x in candidate_items[:120]]
    ...
    def recommend_candidates(self, user_list, allow_repeat=True):
        ...
        def add_row(u_id, idx, score, s_rank, s_sim_max, s_sim_avg, s_support, current_age):
            ...
            # [REMOVED] item_hist_cnt
            f_brand_cnt = 0
            f_cat2_cnt = 0
            f_price_ratio = 1.0
            f_brand_affinity = 0.0
            f_days_since_cat = 999 
            # [REMOVED] f_days_since_item
        ...
        for user_id in tqdm(user_list, desc="Generating Candidates"):
            ...
            if user_id not in self.user_id_to_index_:
                ...
                # 1. Essential (12 items)
                if self.essential_cold_indices_:
                    n_ess = min(len(self.essential_cold_indices_), 12)
                    final_indices.extend(self.essential_cold_indices_[:n_ess])
                
                # 2. Pregnancy/Newborn (3 items)
                ...
                # 3. Global Cold Popularity
                remaining = 20 - len(set(final_indices))
                if remaining > 0 and self.cold_start_pop_indices_:
                    pool = [x for x in self.cold_start_pop_indices_ if x not in final_indices]
