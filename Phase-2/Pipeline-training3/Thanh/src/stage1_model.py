...
class ItemItemCFStage1(BaseEstimator):
    def __init__(self, config, df_item, df_user):
        ...
        self.item_metadata_ = {}   
        self.user_profiles_ = {}   
        # [REMOVED] self.user_item_last_date_ không còn cần thiết
        ...
        # [NEW] ĐỊNH NGHĨA DANH SÁCH FEATURE TẠI ĐÂY (Nguồn duy nhất)
        self.feature_names = [
            # 1. Ranking Info
    ...
    def fit(self, df_train):
        print(f"[{self.__class__.__name__}] Starting fit process (No Recency Features)...")
        ...
        # [NEW] Tự động lưu feature config sau khi fit
        self.save_feature_config() 
        return self

    # [NEW] Hàm lưu file config
    ...
    def _build_advanced_features_data(self):
        ...
        for uid, group in tqdm(df_valid.groupby("customer_id"), desc="Building Profiles"):
            ...
            # Chỉ tính avg_spent để làm Price Ratio, bỏ last_date
            ...
            self.user_profiles_[u_idx] = {
                ...
                "cat2_avg_spent": cat_stats # Chỉ lưu avg spent
            }
    ...
    def recommend_candidates(self, user_list, allow_repeat=True):
        ...
        def add_row(u_id, idx, score, s_rank, s_sim_max, s_sim_avg, s_support, baby_age):
            ...
            rows.append({
                ...
                # "feat_days_since_item": REMOVED
                # "feat_days_since_cat": REMOVED
        ...
        for user_id in tqdm(user_list, desc="Generating Candidates"):
            ...
            for c_idx, score in sorted_cands:
                add_row(user_id, c_idx, score, rank, max_sims[c_idx], score / support_counts[c_idx], support_counts[c_idx], f_baby_age)
            ...
            if len(sorted_cands) < self.top_k:
                for p_idx in self.popular_item_indices_:
                    if p_idx not in added_indices:
                        ...
                        add_row(user_id, p_idx, 0.0, rank, 0.0, 0.0, 0, f_baby_age)
                        rank += 1
